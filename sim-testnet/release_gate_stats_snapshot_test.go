...
func TestProducerGateStateSelectionStatsSnapshotRetainsNativeCustody(t *testing.T) {
	for _, edge := range []struct{ path, caller, callee string }{
		...
		if !releaseClosureFunctionCalls(t, edge.path, edge.caller)[edge.callee] {
			t.Errorf("%s lost Stats snapshot custody edge %s", edge.caller, edge.callee)
		}
	}
	for _, name := range []string{"writeStatsSnapshotOwned", "removeTemporary"} {
		...
		if calls["atomicStateWrite"] || calls["CreateTemp"] || calls["Rename"] || calls["Remove"] {
			t.Errorf("native Stats snapshot %s acquired a pathname write bypass", name)
		}
	}
}
